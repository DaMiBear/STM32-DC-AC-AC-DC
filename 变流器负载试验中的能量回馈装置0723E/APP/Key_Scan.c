...
extern int RefVoltage; //main.c 参考电压
extern int RefCurrent;  //main.c 参考电流
extern float SPWM_m; 
void Key_Dealy()
{
     int n,t;
...
//               OLED_ShowNum(64,6,(int)(num2_result * 10),3,16);
               ...
               if( SPWM_k >= 1300 || SPWM_k <= 0 ) //检查数组越界。
               {
                    OLED_ShowString(0,4,(unsigned char *)"Error!");
                    SPWM_k = 450;
                    SPWM_m = 1;
               }
               OLED_ShowString(0,2,(unsigned char *)"f");        //频率输入完毕显示f
