#include "AD.h"
#include "OLED.h"
unsigned char AD1_a,AD1_b,AD1_c,AD1_d;
...
{  
     ...
     for(i=0;i<8;i++)
     {
          ...
          result1 +=ADC_GetConversionValue(ADC1);
          result2 +=ADC_GetConversionValue(ADC2);
     }
     result1 = result1 >>3;                              //取8次采样的平均值
     ...
     //AD1数据处理
     AD1_Result =  result1*VRatio; //用于反馈 转换成LOAD端电压值
     result1 = result1*VRatio;     //用于显示

     AD1_a = result1/10000;                //ab.cd
     AD1_b = (result1/1000)%10;
     AD1_c = (result1/100)%10;
     AD1_d = (result1/10)%10;
